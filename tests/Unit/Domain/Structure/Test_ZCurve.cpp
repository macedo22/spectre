...
// Test creating initial ElementIds in Z-curve index order. Tests
// domain::initial_element_ids_in_z_curve_order.
template <size_t Dim>
void test_initial_element_ids_in_z_curve_order(
  ...
  for (size_t i = 0; i < element_ids_in_z_curve_order.size(); i++) {
    ...
    CHECK(domain::z_curve_index(element_id) == i);
  }
}

// Test creating initial ElementIds in Z-curve index order for each refinement
// level permutation for dimensions 1, 2, and 3
void test_initial_element_ids_in_z_curve_order_for_refinement_levels(
  ...
  for (size_t i = min_refinement_level; i < max_refinement_level + 1; i++) {
    ...
    test_initial_element_ids_in_z_curve_order(block_refinement_levels_1d);
    ...
    for (size_t j = min_refinement_level; j < max_refinement_level + 1; j++) {
      ...
      test_initial_element_ids_in_z_curve_order(block_refinement_levels_2d);

      for (size_t k = min_refinement_level; k < max_refinement_level + 1; k++) {
        gsl::at(block_refinement_levels_3d, 2) = k;

        test_initial_element_ids_in_z_curve_order(block_refinement_levels_3d);
      }
    }
  }
}
...
SPECTRE_TEST_CASE("Unit.Domain.ZCurve", "[Domain][Unit]") {
  ...
  // Test creating initial ElementIds in Z-curve index order
  test_initial_element_ids_in_z_curve_order_for_refinement_levels(
