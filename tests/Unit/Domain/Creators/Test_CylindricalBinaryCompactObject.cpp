...
std::string create_option_string(
  ...
  return "CylindricalBinaryCompactObject:"
         ...
         inner_sphere_A + inner_sphere_B + outer_shell + time_dependence +
         boundary_conditions;
}
...
void test_parse_errors() {
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{2.0, 0.05, 0.0}}, {-5.0, 0.05, 0.0}, 1.0, 0.4, 1.0, false, 1_st,
          3_st, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{-2.0, 0.05, 0.0}}, {-5.0, 0.05, 0.0}, 1.0, 0.4, 25.0, false, 1_st,
          3_st, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
          create_inner_boundary_condition(), create_outer_boundary_condition(),
          Options::Context{false, {}, 1, 1}),
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{2.0, 0.05, 0.0}}, {-5.0, 0.05, 0.0}, 0.15, 0.4, 25.0, false, 1_st,
          3_st, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
          create_inner_boundary_condition(), create_outer_boundary_condition(),
          Options::Context{false, {}, 1, 1}),
      Catch::Matchers::ContainsSubstring(
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{2.0, 0.05, 0.0}}, {-1.0, 0.05, 0.0}, 1.0, 0.4, 25.0, false, 1_st,
          3_st, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
          create_inner_boundary_condition(), create_outer_boundary_condition(),
          Options::Context{false, {}, 1, 1}),
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{4.0, 0.0, 0.0}}, {-4.0, 0.0, 0.0}, 1.0, 1.0, 25.0, false, 1_st,
          3_st, std::nullopt, std::nullopt, std::nullopt,
          TimeDepOptions{
              0.0, std::nullopt,
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{2.0, 0.05, 0.0}}, {-5.0, 0.05, 0.0}, 1.0, 0.4, 25.0, false, 1_st,
          3_st, std::nullopt, std::nullopt, std::nullopt, std::nullopt, nullptr,
          create_outer_boundary_condition(), Options::Context{false, {}, 1, 1}),
      Catch::Matchers::ContainsSubstring(
...
void test_cylindrical_bbh() {
  ...
  for (auto [include_outer_sphere, include_inner_sphere_A,
    ...
    std::optional<InnerSphereAOptions> inner_sphere_A_opts{};
    if (include_inner_sphere_A) {
      inner_sphere_A_opts = InnerSphereAOptions{false};
    }
    std::optional<InnerSphereBOptions> inner_sphere_B_opts{};
    if (include_inner_sphere_B) {
      inner_sphere_B_opts = InnerSphereBOptions{false};
    }
    ...
    cyl_binary_compact_object = CylBCO{
        ...
        initial_refinement,
        initial_grid_points,
        std::move(inner_sphere_A_opts),
        std::move(inner_sphere_B_opts),
...
// // TODO : make these args better or assert that arg 3, 4 can't be true if 1
// or 2
// // aren't
// void test_spherical_harmonics_inner_spheres(
//     const bool include_inner_sphere_a, const bool include_inner_sphere_b,
//     const bool use_spherical_harmonics_inner_sphere_a,
//     const bool use_spherical_harmonics_inner_sphere_b, const double radius_a,
//     const double radius_b, const double outer_radius) {
//   INFO(
//       "Test CylindricalBinaryCompactObject with "
//       "UseSphericalHarmonics: true for InnerSphereA and/or InnerSphereB");
//   const size_t initial_refinement = 1;
//   const size_t initial_grid_points = 3;

//   std::optional<OuterSphereOptions> inner_sphere_a_opts{};
//   if (include_inner_sphere_a) {
//     inner_sphere_a_opts = InnerSphereAOptions{include_inner_sphere_a};
//   }
//   std::optional<OuterSphereOptions> inner_sphere_b_opts{};
//   if (include_inner_sphere_b) {
//     inner_sphere_b_opts = InnerSphereBOptions{include_inner_sphere_b};
//   }
...
//       initial_refinement,
//       initial_grid_points,
//       // not using spherical harmonics for outer sphere
//       inner_sphere_a_opts,
//       inner_sphere_b_opts,
//       OuterSphereOptions{false},
//       std::nullopt,
//       create_inner_boundary_condition(),
//       create_outer_boundary_condition()};
...
//   const size_t num_blocks_outer_sphere = 18;
...
//   // initial_extents: OuterShell0 should be {3, 3, 2*(3 - 1) + 1} = {3, 3,
//   5}. const auto extents = cbco.initial_extents(); REQUIRE(extents.size() ==
//   num_blocks); CHECK(extents[first_shell_block_number] == std::array<size_t,
//   3>{3, 3, 5});
...
//   CHECK(refinement[first_shell_block_number] == std::array<size_t, 3>{1, 0,
//   0});
...
//   CHECK(sh_block.neighbors().at(Direction<3>::lower_xi()).ids().size() ==
//   18); CHECK(sh_block.neighbors().count(Direction<3>::upper_xi()) == 0);
