...
void test_compute_spatial_ricci_tensor(
  ...
  const auto conformal_factor = pow(get(det_spatial_metric), -1.0 / 6);
  ...
  const auto field_d_up = gr::deriv_inverse_spatial_metric(
      inverse_conformal_spatial_metric, d_spatial_metric);
  ...
  const auto christoffel_second_kind = Ccz4::conformal_christoffel_second_kind(
  ...
  CHECK_ITERABLE_APPROX(expected_ricci_tensor, actual_ricci_tensor);
}
}  // namespace
