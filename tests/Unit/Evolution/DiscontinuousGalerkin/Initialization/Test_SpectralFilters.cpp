...
void test() {
...

  {
    ...
    FilterVec<Dim> filters_a;
    filters_a.push_back(
        make_hypercube<Dim>(std::vector<std::string>{"Block0"}, block_names));
    FilterVec<Dim> filters_b;
    filters_b.push_back(make_hypercube<Dim>(
        std::vector<std::string>{"Block1", "Block2"}, block_names));

    {
      auto box = make_box<Dim>(std::move(filters_a), 0);
      db::mutate_apply<
          evolution::dg::Initialization::SpectralFilters<Dim, TagList>>(
          make_not_null(&box));
      CHECK(db::get<Filters::Tags::SpectralFilter<Dim, TagList>>(box)
    ...
    {
      auto box = make_box<Dim>(std::move(filters_b), 1);
      db::mutate_apply<
          evolution::dg::Initialization::SpectralFilters<Dim, TagList>>(
          make_not_null(&box));
      CHECK(db::get<Filters::Tags::SpectralFilter<Dim, TagList>>(box)
...

  {
    ...
    CHECK_THROWS_WITH(invoke(),
                      Catch::Matchers::ContainsSubstring(
                          "Cannot specify more than one filter per element"));
  }
}
