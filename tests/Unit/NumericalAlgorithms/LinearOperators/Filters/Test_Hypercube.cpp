...
// Same domain layout used by Test_Filtering.cpp: four blocks, two groups.
...
template <size_t Dim>
HypercubeFilter<Dim> make_filter(
  ...
  return HypercubeFilter<Dim>{half_power,
                              enable,
                              blocks_to_filter,
...
void test_construction_and_accessors() {
  ...
  const auto filter =
      make_filter<Dim>(16, true, blocks_opt, false, true, std::nullopt, 3);
  ...
  const auto unrestricted =
      make_filter<Dim>(8, true, std::nullopt, true, false, 1, std::nullopt);
  ...
  const auto base = make_filter<Dim>(4, true, blocks_opt, false, false, 2, 5);
  CHECK(base == make_filter<Dim>(4, true, blocks_opt, false, false, 2, 5));
  CHECK_FALSE(base !=
              make_filter<Dim>(4, true, blocks_opt, false, false, 2, 5));
  CHECK(base != make_filter<Dim>(5, true, blocks_opt, false, false, 2, 5));
  CHECK(base != make_filter<Dim>(4, false, blocks_opt, false, false, 2, 5));
  CHECK(base != make_filter<Dim>(4, true, std::nullopt, false, false, 2, 5));
  CHECK(base != make_filter<Dim>(4, true, blocks_opt, true, false, 2, 5));
  CHECK(base != make_filter<Dim>(4, true, blocks_opt, false, true, 2, 5));
  CHECK(base !=
        make_filter<Dim>(4, true, blocks_opt, false, false, std::nullopt, 5));
  CHECK(base !=
        make_filter<Dim>(4, true, blocks_opt, false, false, 2, std::nullopt));
...
void test_pup_round_trip() {
  INFO("Serialization");
  CAPTURE(Dim);
  const auto filter = make_filter<Dim>(
...
void test_apply_in_volume() {
  ...
  for (size_t num_pts =
    ...
    for (const bool enable : {true, false}) {
      CAPTURE(enable);
      const auto filter =
          make_filter<Dim>(kFilterHalfPower, enable, std::nullopt, false, false,
...
void test_apply_on_boundary() {
  ...
  for (size_t num_pts =
    ...
    const auto filter =
        make_filter<Dim>(kFilterHalfPower, true, std::nullopt, false, false,
                         std::nullopt, std::nullopt);
...
void test_option_parsing() {
  ...
  parser.parse(
      ...
      "    BoundaryFilterOnSubstep: True\n"
      "    VolumeFilterEveryNSteps: 5\n"
      "    BoundaryFilterEveryNSteps: Auto\n");

  const auto parsed =
      parser.template get<OptionTags::Filter<Filter>, Metavars<Dim>>();
  const auto expected = make_filter<Dim>(
  ...
  all_parser.parse(
      ...
      "    BoundaryFilterOnSubstep: False\n"
      "    VolumeFilterEveryNSteps: Auto\n"
      "    BoundaryFilterEveryNSteps: 7\n");
  ...
  CHECK(all_parsed.apply_volume_filter_on_substep());
  CHECK_FALSE(all_parsed.apply_boundary_filter_on_substep());
  CHECK_FALSE(all_parsed.apply_volume_filter_on_this_step(0));
  CHECK(all_parsed.apply_boundary_filter_on_this_step(0));
  CHECK(all_parsed.apply_boundary_filter_on_this_step(7));
  ...
  auto invalid_filter = make_filter<Dim>(
...
void test_supports_mesh() {
  ...
  const auto f1 = make_filter<1>(kFilterHalfPower, true, std::nullopt, false,
                                 false, std::nullopt, std::nullopt);
  const auto f2 = make_filter<2>(kFilterHalfPower, true, std::nullopt, false,
                                 false, std::nullopt, std::nullopt);
  const auto f3 = make_filter<3>(kFilterHalfPower, true, std::nullopt, false,
                                 false, std::nullopt, std::nullopt);
...
void test_is_equal() {
  ...
  const auto a =
      make_filter<Dim>(8, true, blocks, false, false, 2, std::nullopt);
  const auto b =
      make_filter<Dim>(8, true, blocks, false, false, 2, std::nullopt);
  const auto c =
      make_filter<Dim>(16, true, blocks, false, false, 2, std::nullopt);
...
void test_cartoon() {
  ...
  const auto filter = make_filter<1>(kFilterHalfPower, true, std::nullopt,
                                     false, false, std::nullopt, std::nullopt);
