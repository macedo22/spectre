...
void test_is_equal() {
  ...
  const auto a =
      make_filter(1, std::nullopt, std::nullopt, true, blocks, false, false,
                  std::nullopt, std::nullopt);
  const auto b =
      make_filter(1, std::nullopt, std::nullopt, true, blocks, false, false,
                  std::nullopt, std::nullopt);
  const auto c =
      make_filter(2, std::nullopt, std::nullopt, true, blocks, false, false,
                  std::nullopt, std::nullopt);
...
void make_jacobians(
  ...
  // jac_grid_to_inertial == InverseJacobian<Inertial, Grid> in C++ types.
...
void test_apply_in_volume() {
  ...
  const auto expected_after = [&](const size_t num_modes_to_kill,
    ...
    ylm::TensorYlm::fill_filter<Scalar<DataVector>::structure>(
        make_not_null(&filter_matrices.scalar), ell_max, num_modes_to_kill,
        angular_half, ylm::TensorYlm::CoefficientNormalization::Spherepack);
    ylm::TensorYlm::fill_filter<tnsr::i<DataVector, 3>::structure>(
        make_not_null(&filter_matrices.i), ell_max, num_modes_to_kill,
        angular_half, ylm::TensorYlm::CoefficientNormalization::Spherepack);
    ylm::TensorYlm::fill_filter<tnsr::ii<DataVector, 3>::structure>(
        make_not_null(&filter_matrices.ii), ell_max, num_modes_to_kill,
        angular_half, ylm::TensorYlm::CoefficientNormalization::Spherepack);
    ylm::TensorYlm::fill_filter<tnsr::ij<DataVector, 3>::structure>(
        make_not_null(&filter_matrices.ij), ell_max, num_modes_to_kill,
        angular_half, ylm::TensorYlm::CoefficientNormalization::Spherepack);
    ylm::TensorYlm::fill_filter<tnsr::ijj<DataVector, 3>::structure>(
        make_not_null(&filter_matrices.kii), ell_max, num_modes_to_kill,
        angular_half, ylm::TensorYlm::CoefficientNormalization::Spherepack);
...
void test_option_parsing() {
  ...
  parser.parse(
      ...
      "    BoundaryFilterEveryNSteps: None\n");
  const auto parsed =
