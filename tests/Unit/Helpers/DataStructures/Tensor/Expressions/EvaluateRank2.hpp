...
template <auto& TensorIndexA, auto& TensorIndexB, typename DataType,
          typename RhsSymmetry, typename RhsTensorIndexTypeList,
          typename LhsTensorIndexTypeList = RhsTensorIndexTypeList>
void test_evaluate_rank_2_impl_2() {
  ...
  //                             symmetry_21, LhsTensorIndexTypeList>();
  // }
  ...
  if constexpr (std::is_same_v<RhsSymmetry, symmetry_11>) {
    ...
    if constexpr (::tenex::detail::is_generic_spacetime_index_value(
      ...
      // true; symmetry_21; spacetime, spatial
      // false; symmetry_11; spacetime, spacetime
      // false; symmetry_21; spacetime, spacetime
      // test_evaluate_rank_2_core<false, TensorIndexA, TensorIndexB, DataType,
      //                         RhsSymmetry, RhsTensorIndexTypeList,
      //                         symmetry_21, LhsTensorIndexTypeList>();
    }
    ...
    // L_ia = R_ia
    else if constexpr (::tenex::detail::is_generic_spatial_index_value(
                      tensorindex_values[0]) and
                  ::tenex::detail::is_generic_spacetime_index_value(
                      tensorindex_values[1])) {
      // true; symmetry_21; spatial, spacetime
      // false; symmetry_11; spacetime, spacetime
      // false; symmetry_21; spacetime, spacetime
    }

    // L_ij = R_ij
    // TODO : also need to check for both cases - when RHS indices are
    // both spatial and when they're both spacetime
    else if constexpr (::tenex::detail::is_generic_spatial_index_value(
                      tensorindex_values[0]) and
                  ::tenex::detail::is_generic_spatial_index_value(
                      tensorindex_values[1])) {
      // true; symmetry_11; spatial, spatial
      // false; symmetry_11; spacetime, spacetime
      // false; symmetry_21; spatial, spatial
      // false; symmetry_21; spacetime, spacetime
    }
    ...
    // L_ti = R_ti
    else if constexpr (::tenex::detail::is_time_index_value(
                      tensorindex_values[0]) and
                  ::tenex::detail::is_generic_spatial_index_value(
                      tensorindex_values[1])) {
      if constexpr (ReturnLhsTensor) {
