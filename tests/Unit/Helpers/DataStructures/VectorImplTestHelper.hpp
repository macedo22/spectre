...
void vector_test_construct_and_assign(
  ...
  UniformCustomDistribution<tt::get_fundamental_type_t<ValueType>> dist{low,
                                                                        high};
  static_assert(default_vector_impl_static_size >= 2 and
                default_vector_impl_static_size <= 20);
  UniformCustomDistribution<size_t> static_sdist{
      2, default_vector_impl_static_size};
  UniformCustomDistribution<size_t> sdist{default_vector_impl_static_size + 1,
                                          20};

  std::vector<size_t> sizes{static_sdist(gen), default_vector_impl_static_size,
                            sdist(gen)};
...
void test_function_on_vector_operands(
  ...
  static_assert(default_vector_impl_static_size >= 2 and
                default_vector_impl_static_size <= 10);
  UniformCustomDistribution<size_t> static_sdist{
      2, default_vector_impl_static_size};
  UniformCustomDistribution<size_t> sdist{default_vector_impl_static_size + 1,
                                          10};

  std::vector<size_t> sizes{static_sdist(generator),
                            default_vector_impl_static_size, sdist(generator)};
