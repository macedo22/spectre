...
void test(const gsl::not_null<Generator*> generator,
  ...
  TensorExpressions::evaluate<ti_B, ti_c>(make_not_null(&L),
                                          R(ti_B, ti_a, ti_c, ti_A));
}
}  // namespace

SPECTRE_TEST_CASE(
    "Unit.DataStructures.Tensor.Expression.CompileTime.Contract.Rank4to2Dim3DataVector5",
    "[DataStructures][Unit]") {
