...
void test(const gsl::not_null<Generator*> generator,
  ...
  TensorExpressions::evaluate(make_not_null(&L), R(ti_A, ti_b) * S(ti_B, ti_a));
}
}  // namespace

SPECTRE_TEST_CASE(
    "Unit.DataStructures.Tensor.Expression.CompileTime.InnerProduct.2x2to0Dim3DataVector5",
    "[DataStructures][Unit]") {
