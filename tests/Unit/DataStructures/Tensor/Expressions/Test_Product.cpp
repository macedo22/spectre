...
/// \ingroup TestingFrameworkGroup
/// \brief Test the inner product of two rank 3 tensors is correctly evaluated
///
/// \details
/// The inner product case tested is:
/// - \f$L = R^{ab}{}_{i} * S_{b}{}^{i}{}_{a}\f$
///
/// \tparam DataType the type of data being stored in the product operands
template <typename DataType>
void test_inner_product_rank_3x3_operands(
    const DataType& used_for_size) noexcept {
  ...
  // \f$L = R^{ab}{}_{i} * S_{b}{}^{i}{}_{a}\f$
  const Tensor<DataType> actual_product =
      TensorExpressions::evaluate(R(ti_A, ti_B, ti_i) * S(ti_b, ti_I, ti_a));

  DataType expected_product = make_with_value<DataType>(used_for_size, 0.0);
  for (size_t a = 0; a < 4; a++) {
    for (size_t b = 0; b < 4; b++) {
      for (size_t i = 0; i < 2; i++) {
        expected_product += (R.get(a, b, i) * S.get(b, i, a));
      }
    }
  }
