...
void test_tensor_ops_properties() {
  ...
  const auto R_plus_S = R(ti_i, ti_j) + S(ti_i, ti_j);
  const auto R_minus_T = R(ti_i, ti_j) - T(ti_j, ti_i);
  const auto large_expression = R(ti_i, ti_j) + S(ti_i, ti_j) - T(ti_j, ti_i) +
                                S(ti_j, ti_i) + S(ti_i, ti_j) - R(ti_j, ti_i);
