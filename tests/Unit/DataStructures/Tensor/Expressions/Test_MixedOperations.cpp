...
void test_case4(const gsl::not_null<Generator*> generator,
  ...
  TensorExpressions::update<ti_a, ti_b>(
      make_not_null(&actual_result_tensor_filled),
      actual_result_tensor_filled(ti_a, ti_b) +
  ...
  TensorExpressions::update<ti_a, ti_b>(
      make_not_null(&actual_result_tensor_filled),
      actual_result_tensor_filled(ti_a, ti_b) * lapse() +
  ...
  if constexpr (not std::is_same_v<DataType, double>) {
    ...
    TensorExpressions::evaluate<ti_a, ti_b>(
        make_not_null(&actual_result_tensor_temp),
        -spacetime_deriv_gauge_function_temp(ti_a, ti_b) -
            spacetime_deriv_gauge_function_temp(ti_b, ti_a) -
            0.5 * pi_two_normals_temp() * pi_temp(ti_a, ti_b) +
            gamma0_temp() * (normal_spacetime_one_form_temp(ti_a) *
                                 gauge_constraint_temp(ti_b) +
                             normal_spacetime_one_form_temp(ti_b) *
                                 gauge_constraint_temp(ti_a)) -
            gamma0_temp() * spacetime_metric_temp(ti_a, ti_b) *
                normal_dot_gauge_constraint_temp() +
            2.0 * christoffel_second_kind_temp(ti_C, ti_a, ti_b) *
                gauge_function_temp(ti_c) -
            2.0 * pi_temp(ti_a, ti_c) * pi_2_up_temp(ti_b, ti_C) +
            2.0 * phi_3_up_temp(ti_i, ti_a, ti_C) *
                phi_1_up_temp(ti_I, ti_b, ti_c) -
            2.0 * christoffel_first_kind_3_up_temp(ti_a, ti_d, ti_C) *
                christoffel_first_kind_3_up_temp(ti_b, ti_c, ti_D));

    TensorExpressions::update<ti_a, ti_b>(
        make_not_null(&actual_result_tensor_temp),
        actual_result_tensor_temp(ti_a, ti_b) -
            pi_one_normal_temp(ti_j) * phi_1_up_temp(ti_J, ti_a, ti_b));

    TensorExpressions::update<ti_a, ti_b>(
        make_not_null(&actual_result_tensor_temp),
        actual_result_tensor_temp(ti_a, ti_b) -
            inverse_spatial_metric_temp(ti_J, ti_K) *
                d_phi_temp(ti_j, ti_k, ti_a, ti_b));

    TensorExpressions::update<ti_a, ti_b>(
        make_not_null(&actual_result_tensor_temp),
        actual_result_tensor_temp(ti_a, ti_b) * lapse_temp() +
            gamma1gamma2_temp() *
                shift_dot_three_index_constraint_temp(ti_a, ti_b) +
            shift_temp(ti_J) * d_pi_temp(ti_j, ti_a, ti_b));
