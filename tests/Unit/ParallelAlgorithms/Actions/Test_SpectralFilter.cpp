...
// Required PUP factory registration for MockFilter (never actually migrated).
// NOLINTNEXTLINE
PUP::able::PUP_ID MockFilter::my_PUP_ID = 0;

// Tags used in the DataBox
  ...
  using phase_dependent_action_list = tmpl::list<
      Parallel::PhaseActions<
          ...
          tmpl::list<ActionTesting::InitializeDataBox<tmpl::list<
              VarsTag, MeshTag, StepTag, FilterTag, InvJacTag, JacTag>>>>,
...
make_identity_inv_jac() {
  InverseJacobian<DataVector, 1, Frame::Grid, Frame::Inertial> inv_jac(5_st,
                                                                       0.0);
  get<0, 0>(inv_jac) = DataVector(5, 1.0);
  return inv_jac;
}

Jacobian<DataVector, 1, Frame::Grid, Frame::Inertial> make_identity_jac() {
  Jacobian<DataVector, 1, Frame::Grid, Frame::Inertial> jac(5_st, 0.0);
  get<0, 0>(jac) = DataVector(5, 1.0);
  return jac;
}

// MockRuntimeSystem is not movable, so run_action uses a callback to let the
// caller inspect the runner after the action completes.
...
// Returns the scalar DataVector from the Variables in the DataBox.
...
// Downcasts the filter stored in the DataBox back to MockFilter.
// db::get dereferences the unique_ptr, so the tag type is const FilterBase&.
...
// Test: Filters::None is always skipped.
