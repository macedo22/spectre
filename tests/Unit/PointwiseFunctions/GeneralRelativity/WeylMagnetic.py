...
def weyl_magnetic_tensor(grad_extrinsic_curvature, spatial_metric):
    ...
    return ((1.0 / np.sqrt(np.linalg.det(spatial_metric))) * 0.5 *
            (np.einsum("kli,mlk,jm", grad_extrinsic_curvature, e_ijk,
                       spatial_metric) +
             np.einsum("klj,mlk,im", grad_extrinsic_curvature, e_ijk,
                       spatial_metric)))
