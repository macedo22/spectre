...
`DataBox`es are an important SpECTRE data structure and concept that were not addressed by this example, but are essential to most executables. Each parallel component typically has an associated `DataBox` that is a flexible container that holds the information you ask for, specified by `SimpleTag`s and `ComputeTag`s. The motivation for `DataBox`es is outlined [here](https://spectre-code.org/databox_foundations.html) and a guide to using and manipulating them is [here](https://spectre-code.org/group__databoxgroup).
