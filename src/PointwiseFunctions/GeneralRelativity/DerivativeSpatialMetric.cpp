...
void deriv_inverse_spatial_metric(
  ...
  destructive_resize_components(d_inverse_spatial_metric,
                                get_size(get<0, 0>(inverse_spatial_metric)));
  for (auto& component : *d_inverse_spatial_metric) {
    component = 0.0;
  }
      ...
      for (size_t k = 0; k < Dim; ++k) {
        for (size_t m = 0; m < Dim; ++m) {
          for (size_t n = 0; n < Dim; ++n) {
            (*d_inverse_spatial_metric).get(k, i, j) -=
...
tnsr::iJJ<DataType, Dim, Frame> deriv_inverse_spatial_metric(
    const tnsr::II<DataType, Dim, Frame>& inverse_spatial_metric,
    const tnsr::ijj<DataType, Dim, Frame>& d_spatial_metric) noexcept {
  tnsr::iJJ<DataType, Dim, Frame> d_inverse_spatial_metric{};
  deriv_inverse_spatial_metric(make_not_null(&d_inverse_spatial_metric),
