...
void ricci_scalar(
  ...
  *ricci_scalar_result =
  ...
  const auto dimensionality = index_dim<0>(spatial_ricci);
    ...
    for (size_t j = 0; j < dimensionality; ++j) {
      for (size_t k = 0; k < dimensionality; ++k) {
        ricci_up_down.get(j, k) +=
            spatial_ricci.get(i, k) * inverse_metric.get(i, j);
      }
    }
  }
  for (size_t j = 0; j < dimensionality; ++j) {
...
Scalar<DataType> ricci_scalar(
  ...
  ricci_scalar(make_not_null(&ricci_scalar_result), spatial_ricci,
               inverse_metric);
