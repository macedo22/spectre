 ...
 * the typical range 28-32 the angular filter is smooth at the lower turn-on
...
class SphericalShell : public Filter<3, TagList> {
 public:
  ...
  /// \brief Apply the boundary filter once every `N` steps. `None`
  /// (`std::nullopt`) disables the every-N-steps trigger.
  ...
  using options =
      tmpl::list<NumModesToKill, AngularHalfPower, RadialHalfPower, Enable,
                 BlocksToFilter, VolumeFilterOnSubstep, BoundaryFilterOnSubstep,
                 VolumeFilterEveryNSteps, BoundaryFilterEveryNSteps>;
