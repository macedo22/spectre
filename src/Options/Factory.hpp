...
std::unique_ptr<BaseClass> create(const Option& options) {
  ...
  if (node.IsScalar()) {
    ...
    if (node.size() != 1) {
      std::ostringstream ss;
      ss << "Expected a single class to create, got " << node.size() << ":\n"
         << node;
      PARSE_ERROR(derived_opts.context(), ss.str());
    }
    id = node.begin()->first.as<std::string>();
    derived_opts.set_node(node.begin()->second);
  } else if (node.IsNull()) {
    const std::string s{"Expected a class to create:\n" +
                        help_derived<creatable_classes>()};
    PARSE_ERROR(derived_opts.context(), s);
  } else {
    std::ostringstream ss;
    ss << "Expected a class or a class with options, got:\n" << node;
    PARSE_ERROR(derived_opts.context(), ss.str());
  }
  ...
  const std::string s{"Unknown Id '" + id + "'\n" +
                      help_derived<creatable_classes>()};
  PARSE_ERROR(derived_opts.context(), s);
}
}  // namespace Factory_detail
