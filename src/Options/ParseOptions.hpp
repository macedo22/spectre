...
namespace detail {
[[noreturn]] void bad_conversion_error_impl(const YAML::Node& node,
                                            const Context& error_context,
                                            const std::string& yaml_type,
                                            bool is_vector_or_array);
}  // namespace detail
...
#pragma GCC diagnostic pop
    ...
    Options::detail::bad_conversion_error_impl(
        node(), error_context, Options_detail::yaml_type<T>::value(),
        tt::is_a_v<std::vector, T> or tt::is_std_array_v<T>);
