...
void check_for_duplicate_option_name(
    const gsl::not_null<std::vector<std::string>*> result,
...
struct duplicate_option_name_error_helper<
  ...
  static std::vector<std::string> apply() {
    ...
    (check_for_duplicate_option_name(
         make_not_null(&result), pretty_type::name<TopLevelOptionsAndGroups>()),
     ...);
    return result;
  }
};
...
[[noreturn]] void missing_option_error(
    const Options::Context& context,
...
void Parser<OptionList, Group>::parse(const YAML::Node& node) {
  ...
  auto valid_names = call_with_chosen_alternatives([](auto option_list_v) {
    ...
    return detail::duplicate_option_name_error_helper<
  ...
  for (const auto& name_and_value : node) {
    ...
    if (0 != parsed_options_.count(name)) {
      detail::option_specified_twice_error(context, name, parsing_help(node));
    }

    // Check for invalid key
    const auto name_it = alg::find(valid_names, name);
    if (name_it == valid_names.end()) {
      detail::unused_key_error_helper<all_possible_options>::apply(
          context, name, parsing_help(node));
      detail::option_invalid_error(context, name, parsing_help(node));
    }
  ...
  if (not valid_names.empty()) {
    detail::missing_option_error(context_, valid_names, parsing_help(node));
  }
  ...
  tmpl::for_each<subgroups>([this](auto subgroup_v) {
    ...
    subgroup_parser.context_.append(
        detail::add_group_prefix_to_name(pretty_type::name<subgroup>()));
  ...
  if constexpr (std::is_same_v<Group, NoSuchType>) {
    detail::print_top_level_error_message_if_top_level(context_.top_level);
  }
}
