...
struct LineSegment {
  ...
  struct End {
    using type = std::array<double, VolumeDim>;
    Options::String help = {"Ending endpoint"};
  };
  struct NumberOfPoints {
    using type = size_t;
    Options::String help = {"Number of points including endpoints"};
...
struct LineSegment {
  ...
  Options::String help{"Options for interpolation onto line segment."};
