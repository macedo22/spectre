...
struct ElementsAllocator
  ...
  static void apply(Parallel::CProxy_GlobalCache<Metavariables>& global_cache,
    ...
    do {
      ...
      // processors. Note that the quadrature is arbitrarily chosen since it is
      // not in `initialization_items` and will not affect the element
      // distribution given that a uniform distribution does not depend on
      // quadrature
