...
CylindricalBinaryCompactObject::CylindricalBinaryCompactObject(
  ...
  if (time_dependent_options_.has_value()) {
    ...
    // these as member variables or having a function to call for both. Only
    // difference here is that this logic uses the x-aligned centers for EA, EB
    const std::array<double, 3> center_EA_aligned = {
        center_A_aligned[0] * cut_spheres_offset_factor_, 0.0, 0.0};
    const std::array<double, 3> center_EB_aligned = {
        center_B_aligned[0] * cut_spheres_offset_factor_, 0.0, 0.0};
    const double radius_MB = std::abs(
        cut_spheres_offset_factor_ * center_B_aligned[0] - z_cutting_plane_);
    const double radius_EA = sqrt(
        square(center_EA_aligned[0] - z_cutting_plane_) + square(radius_MB));
    const double radius_EB =
        sqrt(2.0) * std::abs(center_EB_aligned[0] - z_cutting_plane_);

    time_dependent_options_->build_maps(
        std::array{center_A_aligned, center_B_aligned}, center_EA_aligned,
        center_EB_aligned,
