...
class Rectangle : public DomainCreator<2> {
 public:
  ...
  struct LowerBound {
    ...
    Options::String help = {"Sequence of [x,y] for lower bounds."};
  };

  struct UpperBound {
    using type = std::array<double, 2>;
    Options::String help = {"Sequence of [x,y] for upper bounds."};
  };
  struct IsPeriodicIn {
    using type = std::array<bool, 2>;
    Options::String help = {"Sequence for [x,y], true if periodic."};
  };

  struct InitialRefinement {
    using type = std::array<size_t, 2>;
    Options::String help = {"Initial refinement level in [x,y]."};
  };

  struct InitialGridPoints {
    using type = std::array<size_t, 2>;
    Options::String help = {"Initial number of grid points in [x,y]."};
  };
  ...
  Options::String help{"Creates a 2D rectangle."};
