...
UniformCylindricalFlatEndcap::UniformCylindricalFlatEndcap(
    const std::array<double, 3>& center_one,
    const std::array<double, 3>& center_two, double radius_one,
    double radius_two_inner, double radius_two_outer, double z_plane_one)
    : center_one_(center_one),
      ...
      radius_two_inner_([&radius_two_inner]() {
        ...
        // so the ASSERT may trigger in the case where we really
        // want an entire domain that is very small.
        ASSERT(not equal_within_roundoff(radius_two_inner, 0.0),
               "Cannot have zero radius_two_inner");
        ASSERT(radius_two_inner > 0.0, "Cannot have negative radius_two_inner");
        return radius_two_inner;
        // codecov doesn't understand lambdas in constructors for some
        // reason, and says the next line is not covered even though it is.
      }()),  // LCOV_EXCL_LINE
      radius_two_outer_([&radius_two_outer]() {
        ...
        // so the ASSERT may trigger in the case where we really
        // want an entire domain that is very small.
        ASSERT(radius_two_outer > radius_two_inner,
               "radius_two_outer must be greater than radius_two_inner but "
               "radius_two_inner="
                   << radius_two_inner
                   << " and radius_two_outer=" << radius_two_outer);
        return radius_two_outer;
        // codecov doesn't understand lambdas in constructors for some
        // reason, and says the next line is not covered even though it is.
      }()),  // LCOV_EXCL_LINE
      z_plane_one_inner_(z_plane_one_inner),
      z_plane_one_outer_([&z_plane_one_inner, &z_plane_one_outer]() {
        ASSERT(z_plane_one_inner > z_plane_one_outer,
               "z_plane_one_inner must be greater than z_plane_one_outer but "
               "z_plane_one_inner="
                   << z_plane_one_inner
                   << " and z_plane_one_outer=" << z_plane_one_outer);
        return z_plane_one_outer;
        // codecov doesn't understand lambdas in constructors for some
        // reason, and says the next line is not covered even though it is.
      }()),  // LCOV_EXCL_LINE
      theta_min_one_([&center_one, &radius_one, &z_plane_one_inner]() {
        const double cos_theta_min =
            (z_plane_one_inner - center_one[2]) / radius_one;
        ASSERT(
            abs(cos_theta_min) < 1.0,
            "Plane one must intersect sphere_one, and at more than one "
            "point. You probably specified a bad value of z_plane_one_inner, "
            "radius_one, or the z component of center_one. "
            "Here z_plane_one_inner="
                << z_plane_one_inner << ", radius_one = " << radius_one
                << ", center_one=" << center_one
                << ", and cos_theta_min = " << cos_theta_min);
        return acos(cos_theta_min);
        // codecov doesn't understand lambdas in constructors for some
        // reason, and says the next line is not covered even though it is.
      }()),  // LCOV_EXCL_LINE
      theta_max_one_([&center_one, &radius_one, &z_plane_one_outer]() {
        const double cos_theta_max =
            (z_plane_one_outer - center_one[2]) / radius_one;
        ASSERT(
            abs(cos_theta_max) < 1.0,
            "Plane one must intersect sphere_one, and at more than one "
            "point. You probably specified a bad value of z_plane_one_outer, "
            "radius_one, or the z component of center_one. "
            "Here z_plane_one_outer="
                << z_plane_one_outer << ", radius_one = " << radius_one
                << ", center_one=" << center_one
  ...
  const auto param_string = [this]() -> std::string {
    ...
    buffer << "\nParameters to UniformCylindricalFlatEndcapShell:\nradius_one="
           << radius_one_ << "\radius_two_inner_=" << radius_two_inner_
           << "\radius_two_outer_=" << radius_two_outer_
  ...
  ASSERT(center_two[2] <= center_one[2] + 5.0 * radius_one,
         "center_two[2] must be <= center_one[2] + 5.0 * radius_one, not "
             ...
             << param_string());
  ASSERT(radius_two_inner >= 0.1 * radius_one * sin(theta_min_one_),
         "radius_two_inner is too small: "
             << radius_two_inner << " "
             << 0.1 * radius_one * sin(theta_min_one_) << param_string());
  ASSERT(radius_two_outer >= 0.1 * radius_one * sin(theta_max_one_),
         "radius_two_outer is too small: "
             << radius_two_outer << " "
             << 0.1 * radius_one * sin(theta_max_one_) << param_string());
  ...
  ASSERT(theta_max_one_ > M_PI * 0.075,
         "z_plane_one is too far from the center of sphere_one: theta/pi = "
             << theta_max_one_ / M_PI << param_string());
  // TODO: need to update this check
  ASSERT(is_uniform_cylindrical_flat_endcap_invertible_on_sphere_one(
             center_one_, center_two_, radius_one_, radius_two_outer_,
             theta_max_one_),
  ...
  const double alpha = atan2(center_two[2] - z_plane_one_outer,
...
UniformCylindricalFlatEndcap::operator()(
  ...
  // (z - 1.0) = = rho_frac = (rho_frac - r_bar_in) / (r_bar_out - r_bar_in)
  // where r_bar_in = 1 and r_bar_in = 2
  // theta_one = theta_min_one_ + (z - 1.0) * (theta_max_one_ - theta_min_one_)
  // Don't need to use sin_ax_over_x() helper because rho_bar will never be 0
  // for a hollow cylinder.
  y = radius_one_ *
          (sin(theta_min_one_ + (z - 1.0) * (theta_max_one_ - theta_min_one_)) /
           z) *
          cylindrical_endcap_helpers::sin_ax_over_x(z, theta_max_one_) *
          (1.0 - zbar) +
      (radius_two_inner_ +
       (z - 1.0) * (radius_two_outer_ - radius_two_inner_)) *
          (1.0 + zbar);
  ...
  //   z = 0.5 *
  //       ((radius_one_ * cos(theta_max_one_ * z) + center_one_[2]) * (1.0 -
  //       zbar) +
  //        center_two_[2] * (1.0 + zbar));
  z = 0.5 * ((radius_one_ * cos(theta_min_one_ +
                                (z - 1.0) * (theta_max_one_ - theta_min_one_)) +
              center_one_[2]) *
                 (1.0 - zbar) +
             center_two_[2] * (1.0 + zbar));
...
std::optional<std::array<double, 3>> UniformCylindricalFlatEndcap::inverse(
  ...
  const auto this_is_small_if_rhobar_is_near_unity = [this, &target_coords]() {
    ...
    const double delta_lambda = -radius_one_ * theta_max_one_ *
                                sin(theta_max_one_) * square(denom) *
  ...
  if (rhobar_min < 1.e-6 and this_is_small_if_rhobar_is_near_zero() < 1.e-14) {
    ...
    const double lambda = (target_coords[2] - center_one_[2] - radius_one_) /
