...
class EveryNIterations : public Trigger {
 public:
  ...
  struct N {
    using type = uint64_t;
    Options::String help{"How frequently to trigger."};
    static type lower_bound() { return 1; }
  };
  struct Offset {
    using type = uint64_t;
    Options::String help{"First iteration to trigger on."};
  };

  using options = tmpl::list<N, Offset>;
  Options::String help{"Trigger every N iterations after a given offset."};
