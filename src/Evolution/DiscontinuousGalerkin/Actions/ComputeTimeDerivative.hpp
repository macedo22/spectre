...
struct ComputeTimeDerivative_impl {
...
template <typename ParallelComponent, typename DbTagsList,
          typename Metavariables>
void ComputeTimeDerivative_impl<Dim, EvolutionSystem, DgStepChoosers,
                                LocalTimeStepping>::
    apply(const gsl::not_null<db::DataBox<DbTagsList>*> box,
  ...
  // We also don't use the number of volume mesh grid points. We instead use
  // the max number of grid points from each face. That way, our allocation
  // will be large enough to hold any face and we can reuse the allocation for
  // each face without having to resize it.
       ...
       &face_temporaries](auto derived_correction_v) {
        ...
        if (typeid(boundary_correction) == typeid(DerivedCorrection)) {
          ...
          detail::internal_mortar_data<EvolutionSystem, Dim>(
              box, make_not_null(&face_temporaries),
          ...
          detail::apply_boundary_conditions_on_all_external_faces<
              EvolutionSystem, Dim>(
              box, dynamic_cast<const DerivedCorrection&>(boundary_correction),
              temporaries, volume_fluxes, partial_derivs, primitive_vars);
        }
      });

  if constexpr (LocalTimeStepping) {
    take_step<EvolutionSystem, LocalTimeStepping, DgStepChoosers>(box);
  }

  send_data_for_fluxes<ParallelComponent>(cache, box, volume_fluxes);
}
...
ComputeTimeDerivative<Dim, EvolutionSystem, DgStepChoosers, LocalTimeStepping>::
  ...
  ComputeTimeDerivative_impl<
      Dim, EvolutionSystem, DgStepChoosers,
      LocalTimeStepping>::template apply<ParallelComponent>(make_not_null(&box),
                                                            make_not_null(
                                                                &cache));
