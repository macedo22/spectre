...
class Weno<VolumeDim, tmpl::list<Tags...>> {
 public:
  ...
  struct Type {
    using type = WenoType;
    static Options::String help = {"Type of WENO limiter"};
  };
  ...
  struct TvbConstant {
    ...
    static Options::String help = {"TVB constant 'm'"};
  };
  ...
  struct DisableForDebugging {
    ...
    static Options::String help = {"Disable the limiter"};
  };
  using options =
      tmpl::list<Type, NeighborWeight, TvbConstant, DisableForDebugging>;
  static Options::String help = {"A WENO limiter for DG"};
