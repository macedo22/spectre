 ...
 public:
  ...
  static Options::String help;  );
    ...
    void operator()(const db::DataBox<DbTags>& box,
      ...
      const auto transform_to_modal =
          [&spin_weighted_data_view, &goldberg_mode_buffer, &box, &l_max,
           ...
           &data_to_write](auto tag_v, const auto& spin_weighted_transform,
            ...
            // Make non-owning ComplexDataVector to angular data corresponding
            // to this radial index
            ...
            // already the proper size (spin_weighted_data_view) than it is to
            // try and do all the indexing into the big block of memory here in
            // this call. That is why we have spin_weighted_data_view above.
            ...
            // SpinWeighted<ComplexModalVector>, but we don't know the spin
            // until we loop over tensors, so we have goldberg_mode_buffer (a
            // ComplexModalVector) allocated properly above and just point into
            // it here.
      ...
      // Scalar<DataVector> because it's only real and only has one set of
      // angular points worth of data to write. However, all the machinery above
      // is for a SpinWeighted<ComplexDataVector>. Luckily there is a
      // ComplexInertialRetardedTime where the real part is the
      // InertialRetardedTime and the imaginary part is 0, so we use that
      // instead swapping the names and legend where necessary
      ...
      // One minus y is also special because every angular grid point for a
      // given radius holds the same value. Thus we only need to write one
      // double per radial grid point corresponding to 1 - y. The subfile name
      // is just the name of the tag, and the column names correspond to the
      // names of the radial subfiles for the spin weighted quantities
      ...
      if (variables_to_observe_.count(one_minus_y_name) == 1) {
        ...
        // first one
        for (size_t radial_index = 0;
             radial_index < number_of_radial_grid_points; radial_index++) {
      ...
      // Everything needs the same time so we just write it once here. We use
      // the code time because the inertial retarded time is specified over the
      // whole sphere and is written above (as a function of the code time as
      // well)
      data_to_write[0] = time;

      // Loop over all available spin weighted tags and check if we are
      // observing this tag. We just capture everything in the scope because we
      // need a majority of the variables anyways
