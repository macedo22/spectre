...
void TimeDerivative<Dim>::apply(
  ...
  *temp_gamma2 = gamma2;
  ...
  // using inverse_spatial_metric_logical_1_t =
  //     Tensor<DataVector, Symmetry<2, 1>,
  //            index_list<SpatialIndex<Dim, UpLo::Up, Frame::ElementLogical>,
  //                       SpatialIndex<Dim, UpLo::Up, Frame::Inertial>>>;
  ...
  // - 30 subs for three_index_constrint = -30 ops
  ...
  // - 10 sums for shift_dot_three_index_constraint = 10 * (3 + 2) = -50 ops
  // - 10 sums for mesh_velocity_dot_three_index_constraint = 10 * (3 + 2) = -50
  // ops
  ...
  for (size_t mu = 0; mu < Dim + 1; ++mu) {
    for (size_t nu = mu; nu < Dim + 1; ++nu) {
      ...
      for (size_t m = 0; m < Dim; ++m) {
        ...
        for (size_t n = 0; n < Dim; ++n) {
          // dt_pi->get(mu, nu) -= inverse_spatial_metric_logical_1.get(m, n) *
...
template <size_t Dim>
void OgTimeDerivative<Dim>::apply(
    const gsl::not_null<tnsr::aa<DataVector, Dim>*> dt_spacetime_metric,
    const gsl::not_null<tnsr::aa<DataVector, Dim>*> dt_pi,
    const gsl::not_null<tnsr::iaa<DataVector, Dim>*> dt_phi,
    const gsl::not_null<Scalar<DataVector>*> temp_gamma1,
...
#define INSTANTIATE(_, data)                                                 \
  ...
  template struct gh::OgTimeDerivative<DIM(data)>;                           \
