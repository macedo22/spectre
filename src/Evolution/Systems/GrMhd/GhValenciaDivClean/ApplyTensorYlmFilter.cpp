...
void apply_tensor_ylm_filter(
  ...
  const size_t N = vars->number_of_grid_points();
  const size_t M = temp_storage->number_of_grid_points();
  ASSERT(N == radial_extents * ylm.physical_size(),
         "Mismatch: vars has " << N << " grid points, expected "
                               << radial_extents * ylm.physical_size());
  ASSERT(M >= radial_extents * ylm.spectral_size(),
         "Mismatch: temp_storage has " << M << " grid points, need at least "
  ...
  Variables<gh_spacetime_vars_list> gh_vars(vars->data(), gh_num_comps * N);
  ...
  Variables<valencia_grid_vars_list> val_grid_vars(temp_storage->data(),
                                                   val_num_comps * N);
  // Spectral Valencia vars in grid frame
  Variables<valencia_grid_vars_list> val_spec_vars(
      temp_storage->data() + val_num_comps * N, val_num_comps * M);
