  ...
  struct DensityCutoff {
    ...
    Options::String help() {
      return "Where the density is below this cutoff the fluid variables are "
             ...
             "Lorentz "
             "factor and enthalpy). "
             "During the evolution, atmosphere treatment will typically kick "
             "in and "
             "fix the value of the fluid variables in these regions. "
             "Therefore, "
             "it makes sense to set this density cutoff to the same value as "
             "the "
    ...
    void set_initial_data(
      ...
      *rest_mass_density = std::move(
      ...
      for (size_t i = 0; i < num_points; ++i) {
        ...
        // Apply the EOS and specific enthalpy only where the density is above
        // the cutoff, because the fluid model breaks down in the zero-density
        // limit
