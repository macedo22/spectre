    ...
    struct PrimitiveVars
      ...
      struct Variables {
        ...
        NumericInitialData(std::string file_glob, std::string subfile_name,
                           std::variant<double, importers::ObservationSelector>
                               observation_value,
                           bool enable_interpolation,
                           PrimitiveVars selected_variables,
        ...
        void select_for_import(
            const gsl::not_null<tuples::TaggedTuple<AllTags...>*> all_fields)
            const {
          // Select the subset of the available variables that we want to read
          // from the volume data file
          tmpl::for_each<primitive_vars_option_tags>(
              [&all_fields, this](const auto option_tag_v) {
                using option_tag =
                ...
                const auto& selected_dataset_name =
                    get<option_tag>(selected_variables_);
        ...
        void set_initial_data(
          ...
          *rest_mass_density = std::move(
              get<hydro::Tags::RestMassDensity<DataVector>>(*numeric_data));
          ...
          for (size_t i = 0; i < num_points; ++i) {
            ...
            // Apply the EOS and specific enthalpy only where the density is
            // above the cutoff, because the fluid model breaks down in the
            // zero-density limit
            if (local_rest_mass_density <= density_cutoff_) {
              ...
              if constexpr (ThermodynamicDim == 1) {
                ...
                get(*pressure)[i] = get(equation_of_state.pressure_from_density(
                    Scalar<double>(local_rest_mass_density)));
                get(*temperature)[i] =
                    get(equation_of_state.temperature_from_density(
                        Scalar<double>(local_rest_mass_density)));
              } else if constexpr (ThermodynamicDim == 2) {
                get(*specific_internal_energy)[i] = get(
                    equation_of_state
                ...
                ERROR(
                    "Only 1d & 2d EOSes implemented for numerical initial data "
                    "right "
                    "now");
              }
       ...
       * grmhd::ValenciaDivClean::Actions::SetNumericInitialData in the action
       * list. See importers::Actions::ReadAllVolumeDataAndDistribute for
       * details, which is invoked by this action.
       */
       ...
       * grmhd::ValenciaDivClean::Actions::ReadNumericInitialData to wait until
       * the data for this element has arrived, and then compute the remaining
       * primitive variables and store them in the DataBox to be used as initial
       * data.
       *
       * This action modifies the tags listed in `hydro::grmhd_tags` in the
       * DataBox (i.e., the hydro primitives). It does not modify conservative
       * variables, so it relies on a primitive-to-conservative update in the
       * action list before the evolution can start.
       *
       * \requires This action requires that the (inverse) spatial metric is
       * available through the DataBox, so it should run after GR initial data
       * has been loaded.
       *
       * \requires This action also requires an equation of state, which is
       * retrieved from the DataBox as `hydro::Tags::EquationOfStateBase`.
       */
      struct SetNumericInitialData {
        ...
        static Parallel::iterable_action_return_t apply(
          ...
          db::mutate<hydro::Tags::RestMassDensity<DataVector>,
              ...
              [&initial_data, &numeric_data, &inv_spatial_metric,
               &equation_of_state](
                  const gsl::not_null<Scalar<DataVector>*> rest_mass_density,
                  const gsl::not_null<Scalar<DataVector>*> electron_fraction,
                ...
                initial_data.set_initial_data(
                    ...
                    div_cleaning_field, lorentz_factor, pressure,
                    specific_enthalpy, temperature,
