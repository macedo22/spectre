...
#include "Executables/Benchmark/General/inner_product_spacetime_3x3/BenchmarkedImpls.hpp"
...
void bench_manual_tensor_equation_lhs_return(benchmark::State& state) {
  ...
  for (auto _ : state) {
    // LHS: result
    const result_type result = BenchmarkImpl::manual_impl_lhs_return(R, S);
...
void bench_manual_tensor_equation_lhs_arg_without_buffer(
  ...
  for (auto _ : state) {
    BenchmarkImpl::manual_impl_lhs_arg(make_not_null(&result), R, S);
    benchmark::DoNotOptimize(result);
    benchmark::ClobberMemory();
  }
}
...
void bench_manual_tensor_equation_lhs_arg_with_buffer(
  ...
  fill_with_random_values(make_not_null(&R), make_not_null(&generator),
                          make_not_null(&distribution));

  // RHS: S
  S_type& S = get<::Tags::TempTensor<2, S_type>>(vars);
  fill_with_random_values(make_not_null(&S), make_not_null(&generator),
                          make_not_null(&distribution));

  // LHS: result
  result_type& result = get<::Tags::TempTensor<0, result_type>>(vars);
