...
void bench_prod_sum_4_plus_equals(benchmark::State& state) {  // NOLINT
...
void bench_prod_sum_4_plus_equals_2_chunk(benchmark::State& state) {  // NOLINT
  const size_t num_grid_points = static_cast<size_t>(state.range(0));
  const DataVector used_for_size =
      DataVector(num_grid_points, std::numeric_limits<double>::signaling_NaN());
  std::uniform_real_distribution<> distribution(0.1, 1.0);
