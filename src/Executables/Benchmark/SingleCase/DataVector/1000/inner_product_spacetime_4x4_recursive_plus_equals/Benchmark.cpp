...
void bench_recursive_plus_equals(benchmark::State& state) {  // NOLINT
  ...
  for (auto _ : state) {
    // LHS: result
    BenchmarkImpl::recursive_plus_equals_impl(make_not_null(&result), R, S);
    benchmark::DoNotOptimize(result);
    benchmark::ClobberMemory();
  }
}

BENCHMARK(bench_recursive_plus_equals);
}  // namespace
