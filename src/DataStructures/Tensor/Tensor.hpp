...
class Tensor<X, Symm, IndexList<Indices...>> {
 ...
 public:
  ...
  SPECTRE_ALWAYS_INLINE constexpr auto operator()(
    ...
    static_assert(
        std::is_same_v<
            tmpl::integral_list<bool, TensorIndices::is_spacetime...>,
            tmpl::integral_list<
    ...
    if constexpr (tmpl::is_set<tmpl::size_t<TensorIndices::value>...>::value) {
      return TE<tmpl::list<TensorIndices...>>(*this);
    } else {
      using rhs_tensorindex_list = tmpl::list<TensorIndices...>;
      using rhs_tensorindex_values =
