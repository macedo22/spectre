...
struct make_tensorindex_list_impl {
  static_assert(
      ...
      "Template parameters of make_tensor_index_list must be TensorIndex "
      "objects.");
  using type = tmpl::list<std::decay_t<decltype(TensorIndices)>...>;
};

template <typename TensorIndexList>
struct make_array_from_tensorindex_list_impl;

template <typename... TensorIndices>
struct make_array_from_tensorindex_list_impl<tmpl::list<TensorIndices...>> {
  static SPECTRE_ALWAYS_INLINE constexpr std::array<size_t,
                                                    sizeof...(TensorIndices)>
  apply() noexcept {
    static_assert((... and tt::is_tensor_index<TensorIndices>::value),
                  "Template parameters of make_array_from_tensorindex_list "
...
/*!
 * \ingroup TensorExpressionsGroup
 * \brief Creates an array of TensorIndex values from a TensorIndex type list
 *
 * @tparam TensorIndexList list of TensorIndex types
 */
template <typename TensorIndexList>
SPECTRE_ALWAYS_INLINE constexpr std::array<size_t,
                                           tmpl::size<TensorIndexList>::value>
make_array_from_tensorindex_list() noexcept {
