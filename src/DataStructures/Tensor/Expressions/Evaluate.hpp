 ...
 * in the RHS tensor expression, i.e. how the expression is written. If you
 * would like to specify the symmetry of the LHS Tensor instead of it being
 * determined by the order of operations in the RHS expression, please use the
 * other `evaluate` overload that takes an empty LHS Tensor as its first
 * argument.
 *
 * ### Example usage
 * Given two rank 2 Tensors `R` and `S` with index order (a, b), add them
