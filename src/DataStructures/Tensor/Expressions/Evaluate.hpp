...
void evaluate(
  ...
  // checks for the data type, symmetry, and index_list are carried out to
  ...
  for (size_t i = 0; i < lhs_tensor_type::size(); i++) {
    (*lhs_tensor)[i] =
        rhs_tensorexpression
            .template get<typename lhs_tensor_type::structure,
