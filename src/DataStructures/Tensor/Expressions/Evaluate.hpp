 ...
 * See `TensorExpressions::evaluate` and use it, instead.
 *
 ...
 * For example, say we do
 * `TensorExpressions::evaluate<ti_a, ti_b>(make_not_null(&L),
 ...
 * use `TensorExpressions::update` instead.
 *
 * ### Example usage
 ...
 * `TensorExpressions::evaluate` overload that takes an empty LHS Tensor as its
 * first argument.
 *
 * ### Example usage
 * Given `Tensor`s `R`, `S`, `T`, `G`, and `H`, we can compute the LHS tensor
 ...
 * it
 *
 * \details See documentation for `TensorExpressions::evaluate` for basic
 * functionality.
 *
 * `TensorExpressions::update` differs from `TensorExpressions::evaluate` in
 * that `TensorExpressions::update` should be used when some LHS `Tensor` has
 * been partially computed, and now we would like to update it with a RHS
 * expression that contains it. In other words, this should be used when we
 * would like to emulate assignment operations like `LHS +=`, `LHS -=`,
 * `LHS *=`, etc.
 *
 * One important difference to note with `TensorExpressions::update` is that it
 * cannot split up the RHS expression and evaluate subtrees, while
 * `TensorExpressions::evaluate` can (see `TensorExpression` documentation).
 * From benchmarking, it was found that the runtime of `DataVector` expressions
 * scales poorly as we increase the number of operations. For this reason, when
 * the data type held by the tensors in the expression is `DataVector`, it's
 * best to avoid passing RHS expressions with a large number of operations (e.g.
 * an inner product that sums over many terms).
 *
 * ### Example usage
 ...
 * initial call to `TensorExpressions::evaluate` followed by any number of calls
 * to `TensorExpressions::update` that use the LHS tensor in the RHS expression
 * and will compute the rest of the equation:
 *
