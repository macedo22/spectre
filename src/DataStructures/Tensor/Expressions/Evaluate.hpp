...
void evaluate_core(
  ...
  if constexpr (is_evaluated_lhs_multi_index(
    ...
    if constexpr (EvaluateSubtrees) {
      // the expression is split up, so evaluate subtrees at splits
      (~rhs_tensorexpression)
          .template evaluate_primary_subtree<rhs_multi_index_list>(
      ...
      if constexpr (not rhs_expression_type::is_primary_start) {
        ...
        (*lhs_tensor)[LhsStorageIndex] =
            (~rhs_tensorexpression)
                .template get_primary<rhs_multi_index_list>(
                    (*lhs_tensor)[LhsStorageIndex]);
      }
    } else {
      // the expression is not split up, so evaluate full expression
      // (*lhs_tensor)[i] = (~rhs_tensorexpression).get(rhs_multi_index);
      (*lhs_tensor)[LhsStorageIndex] =
          (~rhs_tensorexpression).template get<rhs_multi_index_list>();
    }
  }
}
...
void evaluate_impl(
  ...
  if constexpr (EvaluateSubtrees) {
    // Make sure the LHS tensor doesn't also appear in the RHS tensor
    expression(~rhs_tensorexpression)
        .assert_lhs_tensor_not_in_rhs_expression(lhs_tensor);
  }
  ...
  //   // positions of indices in LHS tensor where generic spatial indices are
  //   used
  //   // for spacetime indices
  //   constexpr auto lhs_spatial_spacetime_index_positions =
  //       get_spatial_spacetime_index_positions<LhsIndexList,
  //                                             lhs_tensorindex_list>();
  //   // positions of indices in RHS tensor where generic spatial indices are
  //   used
  //   // for spacetime indices
  //   constexpr auto rhs_spatial_spacetime_index_positions =
  //       get_spatial_spacetime_index_positions<RhsIndexList,
  ...
  //   // positions of indices in LHS tensor where concrete time indices are
  //   used constexpr auto lhs_time_index_positions =
  //       get_time_index_positions<lhs_tensorindex_list>();

  //   (void)std::initializer_list<int>{
  //       (evaluate_core<LhsInts, EvaluateSubtrees>(
  //            lhs_tensor, rhs_tensorexpression, index_transformation,
  //            lhs_spatial_spacetime_index_positions,
  //            rhs_spatial_spacetime_index_positions, lhs_time_index_positions,
  //            std::make_index_sequence<
  //                std::decay_t<decltype(*lhs_tensor)>::size()>{}),
  //        0)...};
  (void)std::initializer_list<int>{
