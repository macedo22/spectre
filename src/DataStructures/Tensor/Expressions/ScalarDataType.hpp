...
struct DoubleAsExpression
    : public TensorExpression<DoubleAsExpression, double, tmpl::list<>,
                              tmpl::list<>, tmpl::list<>> {
  using type = double;
  ...
  /// \brief Create an expression from a double
  DoubleAsExpression(double d) : d_(d) {}
  ~DoubleAsExpression() override = default;

  /// \brief Returns the value represented by the expression
  ///
  /// \details
  /// While a DoubleAsExpression expression does not store a rank 0 Tensor, it
  ...
  SPECTRE_ALWAYS_INLINE const double get(
      const size_t storage_index) const noexcept;

  template <>
  SPECTRE_ALWAYS_INLINE const double get<structure>(
      const size_t /*storage_index*/) const noexcept {
    return d_;
  }

 private:
  /// The value being represented as an expression
  double d_;
};

/// \ingroup TensorExpressionsGroup
/// \brief Defines an expression representing a DataVector
///
/// \details
/// Assumes the DataVector used to construct the expression outlives the
/// expression, itself.
struct DataVectorAsExpression
    : public TensorExpression<DataVectorAsExpression, DataVector, tmpl::list<>,
                              tmpl::list<>, tmpl::list<>> {
  ...
  /// \brief Create an expression from a DataVector
  DataVectorAsExpression(const DataVector& d) : d_(&d) {}
  ~DataVectorAsExpression() override = default;

  /// \brief Returns the value represented by the expression
  ///
  /// \details
  /// While a DataVectorAsExpression expression does not store a rank 0 Tensor,
  /// it does represent one. This is why, unlike other derived TensorExpression
  /// types, there is no second variadic template parameter for the generic
  /// indices. In addition, this is why this template is only instantiated for
  /// the case where `Structure` is equal to the structure of such a Tensor.
  ///
  ...
  /// \return the value of the rank 0 Tensor represented by this expression
  template <typename Structure>
  SPECTRE_ALWAYS_INLINE const DataVector& get(
      const size_t storage_index) const noexcept;

  template <>
  SPECTRE_ALWAYS_INLINE const DataVector& get<structure>(
      const size_t /*storage_index*/) const noexcept {
    return *d_;
  }

 private:
  /// Refers to the value being represented as an expression
  const DataVector* d_ = nullptr;
};
...
//   ScalarDataTypeLValue& operator=(const ScalarDataTypeLValue& other) =
//   default; ScalarDataTypeLValue& operator=(ScalarDataTypeLValue&& other) =
//   default; ~ScalarDataTypeLValue() override = default;

//   /// \brief Returns the value represented by the expression
//   ///
//   /// \details
//   /// While a ScalarDataTypeLValue expression does not store a rank 0 Tensor,
//   it
//   /// does represent one. This is why, unlike other derived TensorExpression
//   /// types, there is no second variadic template parameter for the generic
//   /// indices. In addition, this is why this template is only instantiated
//   for
//   /// the case where `Structure` is equal to the structure of such a Tensor.
//   ///
//   /// \tparam Structure the Structure of the rank 0 Tensor represented by
//   this
//   /// expression
//   /// \return the value of the rank 0 Tensor represented by this expression
//   template <typename Structure>
...
//   ///
//   /// \details
//   /// Instead of deep copying the other ScalarDataTypeRValue, this will
//   simply
...
//   ///
//   /// \details
//   /// This takes ownership of the other ScalarDataTypeRValue's value, which
//   is
...
//   ///
//   /// \details
//   /// While a ScalarDataTypeRValue expression does not store a rank 0 Tensor,
//   it
//   /// does represent one. This is why, unlike other derived TensorExpression
//   /// types, there is no second variadic template parameter for the generic
//   /// indices. In addition, this is why this template is only instantiated
...
//   /// case `DataType==double` and `DataVector{NaN}` in the case
//   `DataType==NaN`. DataType t_;
//   /// Refers to the scalar type value being represented as an expression.
//   /// If the expression was constructed with a scalar rvalue, this refer to
//   the
...
//   /// own `t_`, or another ScalarDataTypeRValue's `t_`, if it was also copy
//   or
//   /// move constructed.
