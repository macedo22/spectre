...
get_index_transformation_and_contracted_pair_positions(
  ...
  // we are contracting `R(ti_A, ti_B, ti_a, ti_b)` and the current multi-index
  // we just accessed (one of the components to sum) is `{0, 0, 0, 0}`,
  // representing \f$R^{00}{}_{00}\f$. To find a next multi-index to sum, we can
  // simply increase one of the pairs' concrete values by 1, e.g. the next
  // multi-index to access could be `{0, 1, 0, 1}` for \f$R^{01}{}_{01}\f$ or
  // `{1, 0, 1, 0}` \f$R^{10}{}_{10}\f$. In this case, the idea is that choosing
  // to increment the concrete values of the rightmost pair could provide better
  // spatial locality, as `{0, 1, 0, 1}` is closer in memory to `{0, 0, 0, 0}`
  // than `{1, 0, 1, 0}` is. It's important to note that this, of course,
  // depends on the implementation of `Tensor_detail::Structure` - specifically,
  // the order in which the components are laid out in memory.
  //
...
struct TensorContract
  ...
  /// \details
  /// Example:
  /// We have expression `R(ti_A, ti_b, ti_a)` to represent the contraction
