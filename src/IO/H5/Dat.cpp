...
// branch of `ERROR` will always call `terminate()` because it throws an
// error. Throwing that error is a code path that will never actually be
// entered at runtime, so we suppress the warning here.
